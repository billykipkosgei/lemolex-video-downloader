...
const setupCookies = () => {
  try {
    ...
    if (process.env.YOUTUBE_COOKIES) {
      ...
      // Check if the environment variable contains a file path or actual cookie content
      let cookieContent = process.env.YOUTUBE_COOKIES;
      
      // If it looks like a file path, log a warning
      if (cookieContent.includes('/path/to/cookies')) {
        logError('⚠️ YOUTUBE_COOKIES contains a placeholder path, not actual cookie content!');
        logInfo('Please paste the actual content of your cookie file into the YOUTUBE_COOKIES environment variable');
      }
      
      // Make sure the cookie file has the proper Netscape format header if not present
      ...
      logInfo(`Cookie file created at: ${cookieFile}`);
    } else {
